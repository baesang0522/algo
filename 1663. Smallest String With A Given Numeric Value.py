...
as the sum of its characters' numeric values. For example, the numeric value of the string "abe" is equal
to 1 + 2 + 5 = 8.
...
class Solution:
    def getSmallestString(self, n: int, k: int) -> str:
        c_dict = {idx + 1: c for idx, c in enumerate(ascii_lowercase)}
        ...
        while n != 0:
            if n != 1:
                if k - c_key[flag] >= n - 1:
                    print(flag)
                    result_list.append(c_dict[c_key[flag]])
                    n -= 1
                    k -= c_key[flag]
                    flag = -1
                else:
